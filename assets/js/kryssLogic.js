    ...
    [].forEach.call(
      ...
      (skuElement) => {
        if (skuElement.innerText === "X-BELOP") {
          ...
          card.getElementsByClassName("card-subtitle")[0].innerText =
            "_____ kr";
...
function updateProductCount(card, event) {
  ...
  document.getElementById("totalPrice").innerText =
    currentTotal.toLocaleString() + " kr";
...
function checkIfTotalExceedsBalance(currentTotal) {
  ...
  if (currentTotal > bankAccount["balance"] && !bankAccount["soci_gold"]) {
    ...
    textField.innerText =
      "- " + (currentTotal - bankAccount["balance"]).toString() + " kr";
...
function buildProductOrder(skuNumber, count, directChargeAmount = null) {
   ...
   */
  let productOrdersArray =
    JSON.parse(sessionStorage.getItem("productOrders")) || [];
  const index = productOrdersArray.findIndex(
    (order) => order.sku === skuNumber
  );
  if (index === -1 && count > 0) {
    ...
    productOrdersArray[index].direct_charge_amount !== undefined &&
    directChargeAmount != null
  ) {
...
function showNumberInput(card) {
  ...
  if (
    isNaN(parseInt(card.getElementsByClassName("card-subtitle")[0].innerText))
  ) {
...
function cancelInput(button) {
  ...
  const currentDirectAmount = parseInt(
    card.getElementsByClassName("card-subtitle")[0].innerText
  );
...
function confirmInput(button) {
  ...
  const newDirectAmount = parseInt(
    card.getElementsByClassName("card-subtitle")[0].innerText
  );
